...
$(document).ready(function (){
  ...
  var cacoTheReferee = new Referee ();


  cacoTheReferee.players.push(player1, player2);
  cacoTheReferee.pickPlayer();
  ...
  $("#gameChoice").click(function (){
    ...
    if (gameChoice === "ai"){
      // console.log("ai");
      cacoTheReferee.players[1].playerID = "ai";
  ...
  setInterval(function (){   //setInterval() method will be used to time events
    if (player2.playerID === "ai" && player2.turn ===1 ){
      if (player2.runningTotal <= 15){
          cacoTheReferee.throw();     //Execution of the current function will stop (the statements after throw won't be executed
          $("#hold").hide();
          $("#roll").hide();
      }else {
        cacoTheReferee.hold();
...
// section of the declarion of the winner
      ...
      cacoTheReferee.checkGame();
      if (cacoTheReferee.gameover === 1){
        $("#winner").show().text(cacoTheReferee.winner + "wins!!!");
      }
      showDice(cacoTheReferee.dice);
